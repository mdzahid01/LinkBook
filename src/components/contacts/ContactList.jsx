...
const ContactList = () => {
    ...
    const handleDeleteCheckedContacts = () => {
        ...
        const updatedContacts = filteredContacts.filter((contact) => checkedContacts.includes(contact.id));
        setContacts(() => contacts.filter((contact) => !updatedContacts.includes(contact)));
        setCheckedContacts([]);
    }
    ...
    useEffect(() => {
        const filtered = contacts.filter((contact) => {
            const isMatchingName = contact.name.toLowerCase().includes(bouncedQuery.toLowerCase());
            const isMatchingEmail = contact.email.toLowerCase().includes(bouncedQuery.toLowerCase());
            const isMatchingPhone = contact.phone.toLowerCase().includes(bouncedQuery.toLowerCase());
    ...
    return (
        <div className='contact-list'>
            <div className="contact-list-header">
                ...
                <button onClick={() => { seeder(); location.reload() }}>seed data</button>
            ...
            {filteredContacts.length>0 && (
                ...
                <input
                    ...
                    onChange={(e) => {
                        if (e.target.checked) {
                            setCheckedContacts(filteredContacts.map((contact) => contact.id));
                        } else {
                            setCheckedContacts([]);
                        }
                    }}
                    checked={checkedContacts.length === filteredContacts.length && filteredContacts.length > 0}
                />
                <label htmlFor={`checkbox-all`} className='label'>Select all</label>
                </div>

                <div>
                    {checkedContacts.length > 0 && (<button className='btn btn-delete' onClick={handleDeleteCheckedContacts}>
                        <FaTrashAlt  /> &nbsp; selected
                    </button>)
                    }
                </div>

                <span className="multiple-selection-text">{checkedContacts.length}/{filteredContacts.length}</span>
